...
export async function connectWallet() { const wallet = getWalletClient(); if (!wallet) throw new Error('No EVM wallet detected'); const [account] = await wallet.requestAddresses(); const state = await readWalletState(); if (!state.address || state.address.toLowerCase() !== account.toLowerCase()) throw new Error('Wallet account changed before connection completed.'); return { wallet, account: normalizeAddress(account) as Address, chainId: state.chainId, isSupportedChain: state.isSupportedChain } }

export async function disconnectWallet() { const state = await readWalletState(); return { ...state, address: null } }
